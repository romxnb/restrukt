...
**Атлас** — повний перелік зовнішньо запущених сценаріїв обраного обсягу. **Семантична трасованість** — ціль коду: людина читає orchestration сценарію тими самими доменними словами й у тому самому причинному порядку, що й історію; правила та інваріанти живуть у глибоких модулях під цією поверхнею; зовнішні системи стоять за портами.
...
flowchart LR
  ...
  P --> AD[Адаптер<br/>драйвер: БД · HTTP · пошта · час · UI]
...
| orchestration | orchestration home | use case, interactor, application service | кроки сценарію в причинному порядку | граф: найвищий вузол, чиє піддерево покриває всі якорі |
| домен | глибокий модуль | entity, aggregate, policy, specification, domain event | правила, інваріанти, події | кошики 2–4 |
| порт | порт | driven port | інтерфейс до зовнішньої системи доменними словами | граф: вузол `?` «інтерфейс без єдиної реалізації» |
| адаптер | адаптер | driven adapter, infrastructure | реалізація порту для драйвера | граф: вузол `⇥`, файл імпортує драйвер зі списку збирача |
...
**Зовнішня система** — driven actor за Кокберном, «IO device» за Мартіном: усе, з чим код говорить не словами домену, а протоколом: база, мережа, файли, годинник, framework UI, стороння бібліотека з побічними діями. З нею працює лише адаптер; orchestration home кличе порт.

Три правила замість тек:

1. orchestration home не імпортує драйверів;
2. глибокий модуль не імпортує нічого поза своїм контекстом;
...
Рядок атласу: `id · контекст · актор/тригер · намір → результат · точка входу · вид · контракт · стан`. Стан `атлас` — намір і результат; `детально` — звірена історія; `опора` — пояснений `In`. Атлас пишеться до читання реалізації як гіпотеза і уточнюється сканерами. Повний reverse engineering завершено, коли `атлас` лишився лише у явно відкладених контекстах.
...
Історія описує причинний граф сценарію. Кожен крок має **семантичний якір** — функцію, use case, команду, подію, policy, specification або порт, за якою крок однозначно знаходиться в коді; один якір може складатися з кількох символів за видимої межі.
...
На графі сценарію кандидат на дім видно механічно: це найвищий вузол, чиє піддерево покриває всі якорі сценарію в межах контексту. Якщо такого вузла немає — якорі висять на різних гілках просто від точки входу — дому немає, і це рядок кошика склад, а не питання смаку.

Спільний доменний модуль, що володіє одним правилом і служить кільком сценаріям контексту, — повторне використання, не знахідка кошика склад. Кілька сценаріїв можуть жити методами одного application service, поки його ім'я чесне. Для choreography end-to-end сценарій має кілька локальних домів, з'єднаних явними подіями; process manager вводиться лише коли домен справді має власника довгого процесу з власним станом.

## Спільні відрізки

Два і більше сценаріїв однієї групи з тими самими якорями в тому самому порядку мають один **спільний відрізок** `Jn`: власні кроки, власна звірка, власний orchestration home. Сценарій-тригер тримає лише свої кроки до відрізка і закінчується `→ J1`; політика Event Storming «коли …, тоді J1» і є таким сценарієм. Відрізок пишеться раз, звіряється раз, кошики його кроків належать йому. Групи складені за спільними файлами, тому спільний відрізок завжди дістається одному сканеру.
...
5. правило або інваріант, спільний для сценаріїв, має один дім;
6. перехід між контекстами виражений командою або подією;
7. технічні деталі не розривають доменну поверхню сценарію;
8. orchestration home не імпортує жодного драйвера зі списку збирача: перевірка ґрепом.
