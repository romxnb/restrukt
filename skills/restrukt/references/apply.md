...
```mermaid
flowchart LR
  P[plan.md · stories/Cn.md<br/>рядки так і змінено] --> K["кроїльник · судження medium<br/>основа → зріз → рецепт"]
  K --> R[(recipe.Cn.md)]
...
| 10 | orchestration home не імпортує драйверів; кожна зовнішня система зрізу стоїть за портом з доменним іменем | ґреп імпортів за списком драйверів із плану |

Telling the Story of the System як остання перевірка: кроїльник читає сценарій із тіла orchestration home, пише в лог ланцюг якорів і звіряє його з історією to-be зі `stories/Cn.md`, крок за кроком. Крок to-be без якоря в тілі, зайвий порядок, прихована межа, правило, продубльоване в orchestration, або драйвер у тілі — пункт 2 або 10 не пройшов.
