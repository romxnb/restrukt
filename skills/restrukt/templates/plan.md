# План restrukt: <проєкт>

<дата> · N файлів · N точок входу · N бізнес-маршрутів · N опор · N контекстів · N пропозицій · тестів N · час: оркестратор N хв · сканери N хв · злиття N хв

Межа атласу: `<application inbound adapters | public API модуля/library>` · обсяг: `<шлях>`

Основа до створення плану: commit `<sha>` або «без git» · робоче дерево без `docs/restrukt/`: чисто або `<git status --short>`

## Атлас маршрутів

Повний для обраного обсягу: кожна точка входу має кореневий бізнес-маршрут `Rn` або опору `In`; матеріально інші результати маршруту є дочірніми `Rnx`. Формат і критерії — `references/routes.md`.

| id | контекст | актор / тригер | намір → результат | точка входу | вид | контракт | стан |
|---|---|---|---|---|---|---|---|
| R1 | C1a оформлення | покупець | оформити кошик → замовлення оформлено | `Checkout.submit@Checkout.ts:40` | основний | `OrderServiceTests.ts` | детально |
| R1x | C1a оформлення | покупець | оформити кошик → замовлення не відвантажено | `Checkout.submit@Checkout.ts:40` | інший результат R1 | немає тесту | детально |
| I1 | C1a оформлення | startup | зареєструвати HTTP route → служить R1 | `router.post@routes.ts:12` | опора | — | опора |

## Історії

Крок: речення без коду · `семантичний якір@файл:рядок` · звірка. Звірка `=` — код каже те саме, що крок; інакше `зараз: якір n → стане: ім'я m · кошик`, сходинки за Белші. Хвости «стане» разом є історією to-be.

```
R1 · Покупець оформлює замовлення
1. Покупець натискає «Оформити» → «замовлення оформлено» · submit()@Checkout.ts:40 · зараз: isDone 2 → стане: orderPlaced 6 · події
2. коли замовлення оформлено, тоді магазин резервує товар на складі · check()@OrderService.ts:88 · зараз: check() 2 → стане: reserveStock() 5 · слова
3. правило перевіряє, чи замовлення дотягує до безкоштовної доставки · OrderService.ts:120, CartView.ts:77 · зараз: два доми → стане: FreeShippingRule 6, ShippingPolicy · правила
4. магазин списує оплату → «оплату проведено» = paymentCaptured ⇢ C2 · PaymentGateway.ts:12 · =
5. замовлення йде на склад · Warehouse.ts:30 · =
R1x · банк відмовив
1. банк повертає відмову → «оплату відхилено» · PaymentGateway.ts:41 · зараз: status = 2 0 → стане: paymentDeclined 6 · події
2. сторінка показує помилку · Checkout.ts:90 · =
```

```mermaid
flowchart LR
  B((Покупець)) -->|1 оформлює| O[замовлення]
  O --> E1(«замовлення оформлено»)
  E1 -->|2 коли…, тоді резервує| S[товар на складі]
  S -->|3 перевіряє доставку| P{правило}
  P -->|4 списує| M[оплата]
  M --> E2(«оплату проведено»)
  E2 -.->|⇢ C2| C2[C2]
  E2 -->|5| W[замовлення на складі]
  M -.->|R1x| X(«оплату відхилено»)
```

## Шляхи

Найменша зв'язана зміна одного маршруту або інваріанта, оцінена до 10 хвилин і максимум 5 рядків, — «один зелений зріз». Більше — шлях є послідовністю зрізів; ризиковий cutover завжди окремо.

| шлях | що входить | зрізи | id |
|---|---|---|---|
| A | слова і події всіх контекстів | 2 | C1a.1–C1a.2 · C2.1–C2.3 |
| B | R1 end-to-end, потім R1x | 2 | C1a.1–C1a.3 · C1a.5 |
| C | контекст за контекстом | 3 | усі |

## Питання до тебе

| # | контекст | питання | варіанти | відповідь |
|---|---|---|---|---|
| Q1 | C1 | `order` означає замовлення покупця і ордер на відвантаження. Розвести? | a `order` + `pickList` · b лишити одне слово | |

## Контексти

Гіпотеза оркестратора, перерізана в злитті за історіями: межа там, де історія міняє слова на той самий об'єкт або несе `⇢`. Контекст без підконтекстів — один рядок; з підконтекстами — рядок на кожен, словник спільний на контекст.

| id | назва | маршрути | файли | гаряча точка | пропозицій |
|---|---|---|---|---|---|
| C1 | замовлення: спільні слова order, stock, shipping | | | | |
| C1a | оформлення | R1 покупець оформлює · R2 менеджер оформлює за покупця | `Checkout.ts`, `OrderService.ts` | `OrderService.ts` 610 × 27 | 14 |
| C1b | відвантаження | R6 склад відвантажує | `Shipping.ts` | `Shipping.ts` 480 × 9 | 9 |
| C2 | оплата | R9 повернення коштів | `PaymentGateway.ts` | `PaymentGateway.ts` 210 × 6 | 5 |

```mermaid
flowchart LR
  subgraph C1 [C1 · замовлення]
    C1a[C1a оформлення<br/>Checkout · OrderService]
    C1b[C1b відвантаження<br/>Shipping]
  end
  subgraph C2 [C2 · оплата]
    PG[PaymentGateway]
  end
  C1a -.->|«замовлення оформлено»| C1b
  PG -.->|«оплату проведено»| C1a
  OS[/OrderService.ts: C1a · C2/] --- C1a
  OS --- PG
```

### Між контекстами

| id | кошик | де | зараз | стане | чому: connascence | стан |
|---|---|---|---|---|---|---|
| X.1 | склад | R1.2, R9.3 · `OrderService.ts` | один файл тримає кроки C1a і C2 | `OrderService` лише C1a; кроки повернення → `RefundService` у C2 | сильна на відстані: 2 контексти, 1 файл | |

## Джерела мови

`CLAUDE.md` · `docs/glossary.md` · `docs/adr/*.md` — або «не виявлено». Словник: N слів заборон · N збігів в обсязі — або «словника немає».

## Тести

Швидкий контракт контексту: `npm test -- OrderServiceTests`, очікувано до 2 хв. Повний suite: `npm test`, N тестів у `tests/…`. Непокриті результати: R1x — «оплату відхилено». Або «швидкого контракту немає» / «тестів немає».

---

## C1a · Оформлення

Контракт: `OrderServiceTests.ts` 12 · `ShippingPolicyTests.ts` 6. Поведінки: після скасування резерв знімається, оплата повертається · …

Гаряча точка: `OrderService.ts` — тримає стан замовлення і резервує товар і рахує доставку: 3 «і» → C1a.4a–C1a.4b.

| id | кошик | де | зараз | стане | чому: connascence | розходиться з | стан |
|---|---|---|---|---|---|---|---|
| C1a.1 | слова | R1.2 · `OrderService.ts:88` | `check()` 2 | `reserveStock()` 5 | Meaning ×4: місця виклику не знають, що буде резервування | `docs/glossary.md:12` «check» уникати | |
| C1a.2 | події | R1.1 · `Checkout.ts:40` | `isDone` 2, прапорець | «замовлення оформлено» = `orderPlaced` 6, збирається в `Checkout.submit()` | Meaning ×3, 2 модулі: `CartView.ts:40`, `Mail.ts:22` | — | |
| C1a.3 | правила | R1.3 · `OrderService.ts:120`, `CartView.ts:77` | `if total >= 1000 && !promo` у двох домах | тип `FreeShippingRule` 6, дім `ShippingPolicy`; «невідомо» → платна | Algorithm ×2, 2 модулі; гілка «невідомо» дозвільна | `docs/adr/0003.md:14 «Free»` — сходинка 2 | |
| C1a.4a | склад | R1.2, R2.1 · `OrderService.ts` | резерв залежить від внутрішнього стану service | ввести `StockReservation` за чинним інтерфейсом; без cutover | сильна на відстані: 2 історії, 2 модулі | — | |
| C1a.4b | склад | R1.2, R2.1 · `OrderService.ts` | резерв пишуть два доми | перевести R1 і R2 на `StockReservation`; cutover | сильна на відстані: 2 історії, 3 обов'язки | — | підтвердити |

```mermaid
flowchart LR
  subgraph зараз
    A[services/OrderService.ts<br/>стан замовлення · резерв · доставка]
  end
  subgraph стане
    B[orders/OrderService.ts<br/>стан замовлення]
    C[orders/StockReservation.ts]
    D[shipping/FreeShippingRule.ts]
  end
  A --> B & C & D
```

Не дійшов: — або перелік файлів.

Стан: порожньо — чекає · `підтвердити` — ризиковий рядок чекає окремого питання · `так` · `ні` · `змінено: …` · `виконано` · `пропущено: …` · `без відповіді`.
